...
{
  private enum State { process, checked, discounted }
 ...
 /**
  * Apply a discount to the current basket
  */
  public void doDiscount() {
	  
	  if(theBasket.getSavings() > 0) {
		  theBasket.setTotalPrice(theBasket.getTotalPrice() + theBasket.getSavings());
		  theBasket.calculateDiscount();
	  }else {
		  theBasket.calculateDiscount();
  ...
  /**
   * Remove items/reduce item quantity from the basket.
   */
  public void doRemove() throws StockException {
	      if(theBasket == null || theBasket.isEmpty() ) {
	    	  throw new NullPointerException("The basket has not been made or is empty - cannot remove null or empty items");
	      }
	      
	      int index = (theBasket.size() - 1);
	      Product lastElement = theBasket.get(index);
	  
		  theBasket.removeItem();
		  theStock.addStock(lastElement.getProductNum(), 1);
